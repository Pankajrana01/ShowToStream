            ...
            if line.isEmpty {
                ...
                if line.hasPrefix("#EXTM3U") {
                    ...
                    do {
                        ...
                        let bandwidthString1 = try line.replace("(.*),(.*)=(\\d+)(.*)", replacement: "$3")
                        let bandwidthString2 = try line.replace("(.*)=(\\d+)(.*),(.*)", replacement: "$3")
                        
                        if let currentMediaPlaylistExist = currentMediaPlaylist {
                            currentMediaPlaylistExist.programId = Int(programIdString) ?? 0
                            currentMediaPlaylistExist.bandwidth = (bandwidthString1 + bandwidthString2)
