...
//  ShowToStream
//
//  Created by Pankaj Rana on 12/01/21.
//
...
class InAppPurchaseHandler: NSObject {
    ...
    fileprivate var productID = "add-dummy-product-id"
    fileprivate var productsRequest = SKProductsRequest()
    fileprivate var fetchProductComplition: (([SKProduct])->Void)?
    
    fileprivate var productToPurchase: SKProduct?
    fileprivate var purchaseProductComplition: ((InAppPurchaseHandlerAlertType, SKProduct?, SKPaymentTransaction?)->Void)?
    ...
    func purchase(product: SKProduct, complition: @escaping ((InAppPurchaseHandlerAlertType, SKProduct?, SKPaymentTransaction?)->Void)) {
        ...
        if self.canMakePurchases() {
            ...
            complition(InAppPurchaseHandlerAlertType.disabled, nil, nil)
        }
    }
    ...
    func fetchAvailableProducts(complition: @escaping (([SKProduct])->Void)){
        ...
        if self.productIds.isEmpty {
            log(InAppPurchaseHandlerAlertType.setProductIds.message)
            fatalError(InAppPurchaseHandlerAlertType.setProductIds.message)
        }
        else {
...
extension InAppPurchaseHandler: SKProductsRequestDelegate, SKPaymentTransactionObserver{
    ...
    func paymentQueueRestoreCompletedTransactionsFinished(_ queue: SKPaymentQueue) {
        if let complition = self.purchaseProductComplition {
            complition(InAppPurchaseHandlerAlertType.restored, nil, nil)
        }
    }
